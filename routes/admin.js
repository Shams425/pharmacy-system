...
router.get("/newSale", (req, res) => {
  const query =
    `SELECT 
      b.*, m.Medicine_Name, s.Supplier_Name 
    FROM 
      batch b 
    INNER JOIN 
      medicine_information m 
    ON 
      b.Medicine_ID = m.ID 
...
router.post("/genericname/create", function (req, res) {
  ...
  req.getValidationResult().then(function (result) {
    if (!result.isEmpty()) {
      ...
      console.log(generic);
      const query = "INSERT INTO drug_generic_name SET ?";
...
router.get("/batch", function (req, res) {
  ...
  const query =
    "SELECT b.*, m.Medicine_Name, s.Supplier_Name FROM batch b INNER JOIN medicine_information m on b.Medicine_ID = m.ID INNER JOIN supplier s on b.Supplier_ID = s.ID";
  db.getData(query, null, function (rows) {
    const data = {
      batch: rows,
      user: req.session.loggedUser,
...
router.post("/manufacturer/create", function (req, res) {
  ...
  //validation
  req
    .checkBody("manufacturer_name", "Manufacturer Name is required")
    .notEmpty();
...
router.get("/medicine", function (req, res) {
  ...
  const query =
    "SELECT m.*, g.Generic_Name, z.Manufacturer_Name, p.Category FROM medicine_information m INNER JOIN drug_generic_name g on m.Generic_ID = g.ID INNER JOIN manufacturer z on m.Manufacturer_ID = z.ID INNER JOIN category p on m.Category_ID = p.ID";
  db.getData(query, null, function (rows) {
    const data = {
      medicine: rows,
      user: req.session.loggedUser,
...
router.get("/usermanagement", function (req, res) {
  ...
  const query =
    "SELECT A.Name,A.Email,A.Gender,A.Date_of_Birth,A.Age,A.Address,A.Contact,A.Blood_Group,A.Marital_Status,A.Join_Date,A.Salary,A.Username,B.Password,B.Usertype FROM user_information A INNER JOIN user_access B ON A.Username=B.Username;";
  db.getData(query, null, function (rows) {
    const data = {
...
router.get("/employeeReport", (req, res) => {
  res.render("employee_report", {
    user: req.session.loggedUser,
  });
});

router.get("/todayPurchases", (req,res) => {
  res.render("today_Purchases", {
    user: req.session.loggedUser
  })
})
...
router.get("/inventoryStatus", (req,res) => {
  res.render("inventory_status", {
    user: req.session.loggedUser
  })
})
